...

{
    ...
    public async Task<AccountInfoReponse> Getprofile(
    string userId,
    CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(user.Role))
        {
            await _userManager.Users
                ...
                .ExecuteUpdateAsync(setters => setters
                    ...
                    .SetProperty(x => x.Section, "Management"),
                    cancellationToken);
            user.Role = "Admin";
    ...
    public async Task<List<AccountInfoReponse>> GetAllConsultantMembers(CancellationToken cancellationToken = default)
    {
        var members = await _userManager.Users
            .Where(x=>x.Role == "consultant" || x.Role == "Consultant")
            .ToListAsync(cancellationToken);
        return members.Adapt<List<AccountInfoReponse>>();
    }


    public async Task<List<AccountInfoReponse>> GetAllEngineerMembers(CancellationToken cancellationToken = default)
    {
        var members = await _userManager.Users
            .Where(x => x.Role == "engineer" || x.Role == "Engineer")
            .ToListAsync(cancellationToken);
        return members.Adapt<List<AccountInfoReponse>>();
    }
    public async Task<List<AccountInfoReponse>> GetAllSubcontractorMembers(CancellationToken cancellationToken = default)
    {
        var members = await _userManager.Users
            .Where(x => x.Role == "subcontractor" || x.Role == "Subcontractor")
            .ToListAsync(cancellationToken);
        return members.Adapt<List<AccountInfoReponse>>();
    }
    public async Task<List<AccountInfoReponse>> GetAllWorkersMembers(CancellationToken cancellationToken = default)
    {
        var members = await _userManager.Users
            .Where(x => x.Role == "worker" || x.Role == "worker")
            .ToListAsync(cancellationToken);
        return members.Adapt<List<AccountInfoReponse>>();
    }
